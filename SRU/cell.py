...
#        self._output_dims = output_dims
    ...
    def build(self, input_shape):
        ...
        if self._recur_dims > 0:
            self.recur_feats_matrix = self.add_weight(shape=(self.state_size, self._recur_dims), initializer='uniform', name='recur_feats_matrix')
            self.recur_feats_bias = self.add_weight(shape=(self._recur_dims,), initializer=keras.initializers.Constant(0), name='recur_feats_bias')
        ...
        self.stats_bias = self.add_weight(shape=(self._num_stats,), initializer=keras.initializers.Constant(0), name='stats_bias')
        ...
        self.output_bias = self.add_weight(shape=(self.output_size,), initializer=keras.initializers.Constant(0), name='output_bias')
    ...
    def call(self, inputs, states):
        ...
        out_state = K.reshape(self._mavg_alphas*state_tensor + (1-self._mavg_alphas)*stats_tensor, [-1, self.state_size])
